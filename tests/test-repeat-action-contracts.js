...
const unresolvedReverse = contracts.createPlan(editor, { reference: def.definitionId, definitionId: def.definitionId, direction: 'instance-to-linked', sourceInstanceId: 'missing-instance' });
assert.strictEqual(unresolvedReverse.valid, false, 'Eine unbekannte auslösende Instanz darf keinen Vorwärtsplan als Fallback erzeugen.');
assert.strictEqual(unresolvedReverse.blocked, true);
assert.ok(unresolvedReverse.issues.some(issue => issue.code === 'REPEAT_ACTION_SOURCE_INSTANCE_UNRESOLVED'), 'Der blockierte Reverse-Plan benötigt eine konkrete Ursache.');
assert.strictEqual(unresolvedReverse.operations.length, 0, 'Ein ungültiger Reverse-Plan darf keine Source→Instance-Operationen erzeugen.');
