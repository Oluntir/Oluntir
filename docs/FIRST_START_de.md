# Erster Start

1. Oluntir 2.2.1 in einen eigenen Ordner entpacken.
2. `index.html` in einem aktuellen Desktop-Browser öffnen.
3. **Ein Monitor**, **Zwei Monitore** oder die Startabfrage wählen.
4. Ein Projekt erstellen oder eine portable `.oluntir`-Sicherung wiederherstellen.
5. Vor umfangreicher Arbeit das Speichern prüfen und eine externe Sicherung erstellen.
6. Für Source-Package-Importe die Importaktionen der Werkzeugleiste verwenden;
   URLs werden über die lokale API verarbeitet, unbekannte Frameworks bleiben
   `analysis-only`.

## Browserberechtigungen

Ordnerzugriff, Pop-ups und Bildschirmplatzierung werden vom Browser kontrolliert. Nur die für das aktuelle Projekt benötigten Berechtigungen erteilen. Wird das Werkzeugfenster blockiert, im Ein-Monitor-Modus weiterarbeiten und es nach Freigabe der Pop-ups über die Werkzeugleiste erneut öffnen.

## Projektordner

Der Bildmanager kann einen physischen Projektordner verbinden. Den Projektstamm auswählen. Oluntir verwendet oder erstellt automatisch `assets/user_upload/`; diesen Unterordner nicht direkt auswählen.
